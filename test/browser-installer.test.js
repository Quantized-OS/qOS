...
test("GitHub release builder emits the exact verified installer assets", (t) => {
  ...
  assert.match(listing.stdout, /^qos-0\.11\.2\//m);
  assert.match(listing.stdout, /^qos-0\.11\.2\/setup\.sh$/m);
  assert.match(listing.stdout, /^qos-0\.11\.2\/web\/index\.html$/m);
  assert.match(listing.stdout, /^qos-0\.11\.2\/web\/install-macos\.sh$/m);
  assert.match(listing.stdout, /^qos-0\.11\.2\/web\/install-windows\.ps1$/m);
  assert.doesNotMatch(listing.stdout, /^qos-0\.11\.2\/install\.sh$/m);
  const manifest = JSON.parse(readFileSync(join(root, "RELEASE.json"), "utf8"));
  assert.equal(manifest.version, "0.11.2");
