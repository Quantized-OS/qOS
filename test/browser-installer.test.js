...
test("GitHub release builder emits the exact verified installer assets", (t) => {
  ...
  assert.match(listing.stdout, /^qos-0\.11\.3\//m);
  assert.match(listing.stdout, /^qos-0\.11\.3\/setup\.sh$/m);
  assert.match(listing.stdout, /^qos-0\.11\.3\/web\/index\.html$/m);
  assert.match(listing.stdout, /^qos-0\.11\.3\/web\/install-macos\.sh$/m);
  assert.match(listing.stdout, /^qos-0\.11\.3\/web\/install-windows\.ps1$/m);
  assert.doesNotMatch(listing.stdout, /^qos-0\.11\.3\/install\.sh$/m);
  const manifest = JSON.parse(readFileSync(join(root, "RELEASE.json"), "utf8"));
  assert.equal(manifest.version, "0.11.3");
